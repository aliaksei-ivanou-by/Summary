...
| Period | August 2012 - January 2020 (about 7.5 years) |
| Delivery company / customer | PELENG, JSC (ОАО «Пеленг»), Minsk - product and design organization; payloads delivered to satellite prime contractors and national space programs |
| Role | Design engineer (инженер-конструктор) in the Space design department, НКУ «Космос» - the entire period in one unit; in practice an algorithm developer and R&D engineer |
...
| Team | Design-bureau structure: optics, electronics, mechanics, software and test engineers working on one instrument; algorithm work done individually or in pairs inside the space department |
...
I spent the whole period in the same unit, the space design department. My work was algorithm development for those instruments, almost entirely in MATLAB. A satellite imager does not produce a finished picture: its focal plane is assembled from several detector arrays, so a frame arrives as segments that must be joined geometrically and matched radiometrically. My first project was the stitching algorithm for that. The next two concerned the downlink: an onboard lossy compression algorithm of the ADPCM family reaching a 3-4x ratio, and then a follow-up that improved the quality achieved at a given ratio by predicting the image type and adapting quantization to it. Both had to fit a very tight onboard computation budget, which is the constraint that shapes every decision in that kind of work.
...
This was the foundation of my engineering career: seven and a half years of algorithm work on real instruments under real hardware constraints, before I moved fully into commercial C++ development.
...
I worked the entire period in one unit, the space design department (НКУ «Космос»), which is where the instrument-facing algorithm work lived.
...
|---|---|---|
| Frame stitching algorithm for satellite imagery | Continuous, seamless imagery from a multi-detector focal plane | First-hand project record; CV history |
| Onboard lossy compression at 3-4x ratio | Made the mission's image volume fit the downlink within the onboard computation budget | First-hand project record; CV history |
| Image-type prediction and adaptive quantization | Better image quality at the same compression ratio and computational cost | First-hand project record; CV history |
| Automated linear resolution assessment | Repeatable, operator-independent measurement replacing a manual flight-test bottleneck | First-hand project record; CV history |
| Radiometric correction of detector arrays | Suppressed detector-induced defects, providing a sound basis for all downstream processing | First-hand project record; CV history |
| Remote-sensing satellite database | Shared, queryable reference data replacing scattered documents | First-hand project record; CV history |
| UAV processing modules: debayering, stabilization, fusion, format conversion | A working image-processing chain for the UAV optical payload | First-hand project record; CV history |
| IMU calibration and frame-coordinate computation | Reliably geo-referenced UAV imagery | First-hand project record; CV history |
| Payload operation across 30+ UAV test flights and satellite in-orbit testing | Validated instrument and algorithm behavior in real conditions | First-hand project record; CV history |
| 7.5 years of algorithm R&D on flown instruments | The engineering foundation for my later commercial C++ career | Career record |

### Resume-Ready Bullets

- Designed and implemented image-processing algorithms in MATLAB for the optoelectronic imaging payloads of Earth remote-sensing satellites and a UAV optical system over 7.5 years in the space design department of an optoelectronic instrument manufacturer.
...
- **General engineering maturity:** seven and a half years of R&D where decisions were irreversible and verification mattered more than iteration speed.
...
- **What was your role exactly?** Design engineer in the space design department, the same unit for the whole 7.5 years. In practice, an algorithm developer and R&D engineer.
...
- **Why did you leave?** After 7.5 years the growth in the role had flattened, and I wanted to move into commercial software development - which I did, starting with C++.

### Confidentiality Boundaries

- **Safe to discuss:** the organization's public product categories, the satellite programs named in open sources, the classes of algorithm I worked on, the engineering constraints and reasoning, and my personal contributions at the level described in this document.
- **Keep private:** instrument specifications and performance figures beyond what is published, algorithm parameters and coefficients, calibration data, design documentation, customer requirements and internal project naming.
- **Frame the work as it was:** civil Earth remote sensing and the imaging side of an optoelectronic instrument organization. I do not discuss the defense-related side of the enterprise, and there is no need to - the imaging and algorithm work stands on its own and is what the role is relevant to.
- Describe the UAV work in general terms - optical payload processing, calibration, flight tests - without program specifics.
...
- `Summary/different/CV_Scand.doc`: the most detailed primary record - the project table with dates, technologies and role for each PELENG project (frame stitching 08/2012-08/2013; ADPCM compression 08/2013-02/2014; compression quality improvement 02/2014-08/2014; linear resolution determination 08/2014-03/2016; remote-sensing satellite database 02/2015-02/2016; UAV optical system modules 08/2018), plus the position title (инженер-конструктор) and responsibilities summary.
- `Summary/different/Aliaksei Ivanou_CV.pdf`: "Design Engineer, Space Division"; the task list including satellite program participation, flight tests and operation control, ADPCM compression, design projects, imaging planning with Systems Tool Kit, linear resolution analysis, gauge coefficient correction for photomatrices, database development, photo/video processing algorithms, image sensor alignment and UAV IMU calibration.
- `Summary/different/Aliaksei Ivanou_ML.pdf`: motivation letter describing 7.5 years as R&D engineer in the space industry, the satellite programs, the breadth of tasks and MATLAB plus orbit-modelling tools as the main toolset.
- `Summary/different/CV_Europass.pdf`: period 01 Aug 2012 - 31 Dec 2019; technologies MATLAB, Ruby on Rails, System Tool Kit, MS Access, PostgreSQL; participation in optical sensor development for satellites and UAV, flight tests and operation control, and photo/video processing algorithms.
- `History/CV GoogleDocs 03.pdf` through `32.pdf`: successive framings of the role (R&D Engineer, Design Engineer, MATLAB Software Engineer, MATLAB Algorithm Developer), the 08/2012 - 01/2020 period as stated in current CVs, and the algorithm list - feature-based image stitching with keypoint matching, defect removal, brightness uniformity, ADPCM compression at 3-4x, image-type prediction for quantization, automated resolution assessment, debayering, stabilization and sensor fusion, 30+ UAV test flights as payload operator, IMU calibration and frame-centre coordinate computation.
- `Interview/SelfIntro.md`: PELENG as the first long-term role, about 7.5 years as a design and research engineer, MATLAB and internal data-processing tools, and the algorithm set framed as the foundation before commercial C/C++ development.
- `Certificates/2011 Advanced technologies for nanosatellite's experiments in space - 2011.pdf` and `2013 ...- 2013.pdf`: Summer Space School at Samara State Aerospace University during the PELENG period.
- Current project account: the entire period spent in one unit, НКУ «Космос» (the space design department).
...
|---|---|---|
| Employment August 2012 - January 2020 at PELENG, JSC | CV history (CV 26-32); Europass and SCAND questionnaire give December 2019 | Public |
| Position: design engineer (инженер-конструктор), space design department | `Aliaksei Ivanou_CV.pdf` ("Design Engineer, Space Division"); `CV_Scand.doc` | Public |
| Entire period worked in НКУ «Космос» | First-hand project account | Public |
...
| Participation in development, flight tests and operation control for BKA, EgyptSat-2, EgyptSat-A, Kanopus-V series, Obzor-O, Monitoring-SG, Meteor-MP and a UAV optical system | `Aliaksei Ivanou_CV.pdf`; `Aliaksei Ivanou_ML.pdf` | Public at program-name level |
| Frame stitching algorithm, 08/2012 - 08/2013, MATLAB, designer and developer | `CV_Scand.doc` project table | Public |
| ADPCM compression algorithm, 08/2013 - 02/2014, MATLAB; 3-4x ratio | `CV_Scand.doc`; CV history | Public |
| Compression quality improvement / image-type prediction for quantization, 02/2014 - 08/2014, MATLAB | `CV_Scand.doc`; CV history | Public |
| Linear resolution determination software, 08/2014 - 03/2016, MATLAB | `CV_Scand.doc`; CV history | Public |
| Remote-sensing satellite database, 02/2015 - 02/2016, Ruby, Ruby on Rails, PostgreSQL | `CV_Scand.doc` | Public |
| UAV optical system modules (debayering, format conversion, stabilization, frame fusion), 08/2018, MATLAB | `CV_Scand.doc`; CV history | Public |
| 30+ UAV test flights as payload operator; IMU calibration and frame-centre coordinate computation | CV history | Public |
| Gauge coefficient correction for photodetector arrays; image sensor alignment in one plane; imaging planning with Systems Tool Kit; design documentation | `Aliaksei Ivanou_CV.pdf` | Public |
