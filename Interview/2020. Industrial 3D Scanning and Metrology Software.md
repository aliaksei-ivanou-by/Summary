...
| Confidentiality | Company profile, product category, technology stack and engineering approach are safe to discuss; sensor calibration data, internal protocol details, customer identities and medical-application specifics remain private |
...
- **Technical detail:** two sensors is the sparsest arrangement that can produce a 3D surface, so coverage and occlusion dominate: the placement of the two planes determines what can be reconstructed at all, and an organic, curved, non-industrial surface behaves differently from a machined part under laser triangulation.
...
|---|---|---|
| C++17/Qt prototype producing 3D models from multiple 2D laser scanners | Demonstrated that existing standard sensors could be combined into a 3D measurement setup in software | First-hand project record; CV history |
...
| Transition from MATLAB research to commercial C++ | Converted seven years of algorithm experience into production-oriented C++ practice | CV history; career record |
...
- Safe to discuss: the company's public product profile, the technology stack, the architecture and pipeline approach, the engineering challenges and my personal contributions.
- Keep private: calibration data and methodology detail, internal protocol and data-format specifics, source code, internal project naming, end-customer identities and the specifics of the medical application and any associated data.
- Present the medical configuration as a limited exploratory contribution, which is what it was.
...
- `History/CV GoogleDocs 03.pdf`, `05.pdf`, `06.pdf`: RIFTEK period (March 2020 - May 2020), company description, and 3D model generation from multiple 2D laser sensors with PCL and Open3D.
- `History/CV GoogleDocs 07.pdf`, `08.pdf`: C++ / Qt / Point Cloud Library stack and 3D model generation from multiple 2D laser sensors.
- `History/CV GoogleDocs 27.pdf`, `28.pdf`: C++17, Qt, PCL, Git/GitHub, test-driven development; industrial software and robotics domains; the R&D prototype framing and the transition from MATLAB-based research to C++ production code.
- `History/CV GoogleDocs 32.pdf`: current phrasing - "Industrial metrology and 3D scanning software", "3D reconstruction R&D prototype using C++17, Qt, and Point Cloud Library (PCL)".
- `Summary/different/CV_Europass.pdf`: "Development of software for obtaining a 3D model using several 2D laser sensors"; C++, PCL, Open3D, Visual Studio.
- `Interview/SelfIntro.md`: RIFTEK as the first commercial C++ role; C++17/Qt/PCL; industrial metrology and 3D reconstruction prototype processing real measurement and geometry data.
...
|---|---|---|
| Employment period March 2020 - May 2020 | CV history (03, 05, 06, 27, 28, 32), Europass CV | Public |
| RIFTEK develops and manufactures optoelectronic instruments for measuring geometrical quantities, including laser sensors and 2D/3D scanners | CV history; [riftek.com](https://riftek.com/) | Public |
...
| Software generated 3D models from several 2D laser sensors | CV history (03, 05, 06, 07, 08, 27), Europass CV | Public |
| Stack: C++17, Qt, Point Cloud Library, Open3D, Visual Studio, Git | CV history (05, 07, 08, 27, 28, 32), Europass CV | Public |
| R&D prototype facilitating the transition from MATLAB-based research to C++ production code | CV history (28), SelfIntro | Public |
| Sensors were standard RIFTEK production 2D scanners | First-hand project account | Safe internal |
| Two 2D scanners in different planes formed an exploratory 3D scanner for a medical application, intended for prosthetics | First-hand project account | Safe internal - describe at this level of generality only |
