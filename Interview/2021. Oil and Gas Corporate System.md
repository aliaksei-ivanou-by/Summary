...
| Role | Junior C++ Software Engineer at EPAM Systems Belarus; primarily C++ developer with occasional JavaScript work. This was my first project after joining EPAM in September 2021 |
...
| Main stack | C++17, JavaScript with HTML and jQuery, MySQL and Microsoft SQL Server, Nginx, Microsoft Azure |
...
| Confidentiality | Public customer context plus non-customer-specific engineering detail; source code, private data, credentials, endpoints and infrastructure identifiers remain confidential |
...
**Scale.** The customer is a Fortune 500 oilfield-services corporation operating in more than 120 countries. This was internal corporate software at an organization of that size, so the constraint was never novelty - it was that established workflows across a very large company had to keep working through every change.
...
5. JavaScript, with HTML and jQuery on the front end and Nginx serving it, exposed the data through the product's user-facing workflows.

Relational storage spanned MySQL and Microsoft SQL Server, with MySQL carrying the processing workflows I worked on.
...
| MySQL and Microsoft SQL Server | Relational storage for application and processed data; MySQL carried the main processing workflows | Worked with database-backed application workflows |
| Nginx | Served the user-facing layer | Encountered during deployment and diagnosis |
...
| User-facing integration | JavaScript, HTML, jQuery | Product behavior and user-facing pages connected to the C++ codebase | Occasional feature and defect work |
| Persistence | MySQL, Microsoft SQL Server | Relational application and processed-data storage | Application-level integration |
| Web serving | Nginx | Served the application's user-facing layer | Consumer; encountered in deployment and diagnosis |
| Cloud runtime | Microsoft Azure | Hosted application environment | Developer-side use |
| Diagnostics | Application logs; `gdb` and Valgrind available in the native toolchain | Reproduction and cross-layer defect analysis, with native tools available when required | Regular log-based diagnosis; no specific retained `gdb` or Valgrind case |
...
- Bi-weekly sprint demos presented to customer department heads.

I joined EPAM Systems Belarus in September 2021 as a Junior C++ Software Engineer, and this was my first assignment there - which is part of why the three months mattered: it was both an unfamiliar domain and my first enterprise C++ delivery environment. I was promoted to middle grade in May 2022, on the project that followed.
...
Delivery ran on a sprint cycle, with bi-weekly demos presented to department heads on the customer side. For a three-month assignment that shaped how work was sized: each sprint had to produce something demonstrable to people who were not engineers, which favors completed, visible changes over long refactoring efforts.
...
Over the three-month assignment:

- 4+ user-requested features developed and integrated.
- 20+ defects fixed across C++ and JavaScript.
- Refactoring and removal of obsolete code across the areas worked on.
...
- The customer, domain, stack, team structure and engineering approach are all describable.
- Source code, database schemas, sensor payloads, credentials, private endpoints and infrastructure identifiers are confidential.
- My responsibility was application development; customer platform ownership and cloud administration were not mine.
