...
| Documentation | LaTeX (formal course, 2020), standard academic formats | Thesis and coursework preparation | Applied |
...
The other supplementary schools were a Youth Scientific School in Kostroma in October 2009 and the International Summer Space School at Samara State Aerospace University, now Samara University, in August 2011 - "Advanced technologies for nanosatellite's experiments in space", 3.5 ECTS. I returned to the Samara school in August 2013, already working at PELENG, for a second 3.5 ECTS.

The Samara school is worth describing rather than listing. Samara has run it since 2003 with support from the International Astronautical Federation and the Volga branch of the Tsiolkovsky Russian Academy of Cosmonautics. Lectures are given **in English**, the lecturers are international, and the intake is international - students come from across Europe and beyond, selected by application. The programme covers nanosatellite design, CubeSat-class platforms and space experiments, with practical work assembling and launching an atmospheric probe. For me it was the first sustained technical work in English and the first time studying alongside engineers from other countries, five years before English became my working language at EPAM.
...
| Diploma thesis on ozone vertical profile retrieval | Demonstrated feasibility of a new retrieval approach; extended parameterization model, working program, modelled spectra, identified target functions | `Articles/Определение концентрации озона в атмосфере.pdf` |
...
| EM / Gaussian mixture clustering implemented and validated in MATLAB | Practical unsupervised-clustering implementation with synthetic-data validation, in 2011 | `Articles/Алгоритмы выделения однородных участков...pdf` |
...
| Review of transient luminous events in the upper atmosphere | Coherent physical account assembled from a fragmented literature | `Articles/Транзиентные световые явления в атмосфере.pdf` |
| Summer Space Schools, Samara State Aerospace University (2011, 2013) | Space-systems specialization outside the curriculum | `Certificates/2011 ...`, `Certificates/2013 ...` |
...
- Attended the Summer Space School at Samara State Aerospace University on advanced technologies for nanosatellite experiments in space (2011, 2013).
...
- MATLAB as a working tool, which became my primary environment for the following seven and a half years.
...
The `Articles/` folder holds work from two different periods and they should not be read as one set. Three papers are from the degree - the diploma thesis and the two term projects, listed below. The three sorbent and metal-ion papers predate it: they are school-age work from 2005-2007, done in a university laboratory before enrolment, and they are chemistry rather than radiophysics. They belong to `1996-2007. School Years and Pre-University Research.md`, which covers them and the school-age certificates in full.

**From the degree, 2010-2012**

- `Articles/Определение концентрации озона в атмосфере.pdf` - "Experimental and Theoretical Determination of Vertical Ozone Concentration Profiles in the Atmosphere" («Экспериментальное и теоретическое определение вертикальных профилей концентрации озона в атмосфере»), diploma thesis, 5th year, BSU Faculty of Radiophysics and Computer Technologies, Department of Physics and Aerospace Technologies, Minsk 2012. 49 pages, 38 figures, 3 tables, 56 sources. Supervisors E. V. Verkhoturova and A. G. Svetashev; reviewer D. A. Strikelev. Abstract states the objective, the Aladin model extension, the profile-generation program, the parameter influence analysis, the modelled irradiance spectra and the identified parameterization target functions, and names the state programme "Monitoring of the Polar Regions of the Earth and Support of Arctic and Antarctic Expeditions, 2011-2015" («Мониторинг полярных районов Земли и обеспечение деятельности арктических и антарктических экспедиций на 2011-2015 годы»).
- `Articles/Алгоритмы выделения однородных участков по урожайности в точном земледелии.pdf` - "Algorithms for Delineating Homogeneous Yield Zones in Precision Agriculture", 4th year coursework, Minsk 2011, 39 pages. Supervisor E. V. Verkhoturova. Contains the precision-agriculture subsystem overview, the bioequivalence algorithm (sections 3.1.1), the EM/Gaussian mixture zoning algorithm (3.1.2) and an appendix with the MATLAB implementation (`ExpMin.m`, `DistMatrix.m`, `main.m`).
- `Articles/Транзиентные световые явления в атмосфере.pdf` - "Transient Luminous Events in the Upper Atmosphere" («Транзиентные световые явления в верхних слоях атмосферы»), 3rd year coursework, BSU Faculty of Radiophysics and Electronics, Department of Physics, Minsk 2010, 26 pages. Supervisor E. V. Verkhoturova.

**Other repository material**

- `Certificates/2011 Advanced technologies for nanosatellite's experiments in space - 2011.pdf` and `2013 ... - 2013.pdf` - Summer Space School, Samara State Aerospace University.
- `Certificates/2010 LabVIEW. Основы I.pdf` (LabVIEW Basics I) and `2011 LabVIEW. Системы сбора данных.pdf` (LabVIEW Data Acquisition Systems) - measurement and data-acquisition courses at BSUIR.
...
| Diploma title, scope (49 pp, 38 figures, 3 tables, 56 sources), objective, Aladin model extension, profile-generation program, parameter analysis, modelled spectra, target functions | `Articles/Определение концентрации озона в атмосфере.pdf`, abstract | Public |
...
| Bioequivalence algorithm using Monte Carlo simulation from fitted normal distributions | `Articles/Алгоритмы выделения...pdf`, section 3.1.1 | Public |
...
| Transient luminous events review, 3rd year, 2010, 26 pages | `Articles/Транзиентные световые явления в атмосфере.pdf` | Public |
| Summer Space School, Samara, 2011 and 2013 | `Certificates/` | Public |
