...
| Users / deployment | Library staff and API/integration consumers; production services deployed on AWS EC2 Linux hosts |
| Main stack | C89, Java 8/11, Scala, Swagger, PostgreSQL, MySQL, custom protocols, Jira, Bitbucket, Jenkins, Linux on AWS EC2 |
...
I worked primarily as a C developer on Sierra, sharing responsibility for the full C89 codebase with two other C engineers. I implemented features, fixed defects and refactored modules across the backend. Scala APIs invoked C behavior, while database-dependent C code called Java data services that worked with PostgreSQL and residual MySQL models. I occasionally changed Java and Scala when a task crossed component boundaries. Logs were the main diagnostic tool; for native failures I used SSH, `gdb` and core dumps on the AWS EC2 Linux hosts - ordinary Linux machines reached over SSH, not a managed service.
...
Innovative Interfaces is a library-software vendor founded in 1978 and now part of Clarivate. The product line therefore has roots going back more than forty years, and the C89 core carries that history - which is the single most useful fact to state when describing this project, because it explains every constraint that follows. Its product portfolio serves academic, public, national, consortium, corporate and special libraries. My work was delivered by EPAM in direct collaboration with Innovative developers, their development manager and a Product Owner.

**Scale.** The customer's installed library-software base served more than 9,000 libraries worldwide; Sierra was the specific platform on which I worked. The figure describes the scale of the customer's library deployments rather than claiming that every institution ran the same Sierra version or configuration. That scale is the reason compatibility dominated every decision here: a behavior change that suits one library can become a support incident across thousands of institutions, each with its own workflows, integrations and decades of accumulated data.
...
| Web / serving | Apache | Part of the deployed service stack | Consumer; encountered in deployment and diagnosis |
| Diagnostics | Service logs, added logging, SSH, `gdb`, core dumps, Valgrind | Cross-service and native failure analysis; Valgrind for invalid-memory and initialization defects in C | Regular hands-on use |
...
### Role Progression During This Project

Both the promotion and the relocation happened while I was on this project, which makes it the right place to set out the progression at EPAM.

| From | To | Title | Entity | Location |
|---|---|---|---|---|
...
I joined Sierra in March 2022 as a Junior C++ Software Engineer, was promoted to middle grade in May 2022 - eight months after joining EPAM - and moved to EPAM's Polish entity in August 2022 - an internal relocation from Minsk to Warsaw, continuing on the same project without interruption. The customer and the codebase did not change; the employing entity and my location did.

That continuity is worth stating: the promotion to middle grade happened while I was on this project, and the relocation did not cost the project a handover.

### Beyond the Codebase

- Worked as one of the 2-3 C developers on the group's feature work, taking my own tasks through to the components a feature touched.
- Conducted code reviews as part of the mandatory cross-company review process.
- Supported onboarding for new developers joining the team.
- Conducted technical interviews for EPAM.
- Presented at sprint demos to the customer group.
...
| Promoted from Junior to middle-grade C++ Software Engineer, May 2022 | Recognition of independent delivery eight months after joining EPAM |
| Internal relocation to EPAM Systems Poland, August 2022 | Continued on the same project across an entity and country change without a handover |
| Code review, onboarding and technical interviews | Contributed beyond individual delivery as the team changed composition |

### Volume of Work

On this project:

- 12+ user-requested features designed and implemented.
- Defects fixed across the C backend, with related corrections in Java and Scala.

Across the full EPAM period, covering both this project and the oil-and-gas system: 100+ defects fixed spanning C, C++, Java, JavaScript and Scala, and 16+ new features developed and integrated into existing architecture.
...
- Promoted from Junior to middle-grade C++ Software Engineer in May 2022 and relocated internally from EPAM Systems Belarus to EPAM Systems Poland in August 2022, continuing on the same project throughout.
- Contributed code reviews, onboarding support for new developers and technical interviews alongside individual delivery.
...
- Product architecture is describable at the language and service level, and the defect examples in this document are sanitized.
- Custom protocol fields, customer data, private endpoints, hostnames, credentials and infrastructure identifiers are confidential.
- The Java and Scala work was secondary cross-component contribution, and the infrastructure was an integrated platform rather than anything I owned.
...
- `SelfIntro.md`: C89, Java, Scala, custom protocols, Linux, feature work, refactoring, logs, `gdb` and core dumps.
