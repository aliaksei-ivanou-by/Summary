...
| Team | Our project delivery team grew from 3 to 4 C++ engineers; customer PM, approximately 2-4 C++ developers, 1 web developer and usually 2-4 QA engineers; staffing varied during the project |
| Users / deployment / scale | Managed office, reception, dispatcher, meeting-room and contact-center scenarios |
| Main stack | C++17, Qt 5.15, Linphone/liblinphone, SIP/RTP, Yocto, Embedded Linux, JavaScript/Node.js with mediasoup, C#/.NET 8, WinForms |
...
The platform powered physical enterprise SIP phones with account registration, calls, BLF and speed dial, contacts, audio/video conferencing, network and certificate settings, browser-based administration and centralized provisioning. It also included a mediasoup-based conferencing server in JavaScript/Node.js. I mainly developed the embedded C++/Qt application, where I connected asynchronous Linphone call and registration events to MVC/MVVM-style models and UI controllers, but the work frequently crossed into the customized Linphone SDK, Yocto recipes and target-device runtime; I also made a limited contribution to the initial conferencing-server implementation.
...
| Node.js conferencing server | WebRTC conferencing through mediasoup | JavaScript/Node.js, mediasoup and phone-side WebRTC interfaces | Limited contribution to the initial server implementation; primary ownership remained the C++ phone-side integration, and I did not own the later server rewrite | Project and repository history |
...
- **Contributed to and integrated:** Yocto recipes/image contents, firmware startup, media and device behavior, Linphone compatibility fixes, settings/provisioning flows, phone-side WebRTC/mediasoup integration and limited early JavaScript/Node.js conferencing-server work.
...
On our side, the project delivery team started with 3 C++ engineers, including me, and later grew to 4. On the customer side, the team included a project manager, roughly 2-4 C++ developers, one web developer, and usually 2-4 QA engineers, with some turnover during the project.

Customer-side staffing varied during the project, so the figures above describe the typical team composition. Separately from this customer delivery team, the internal engineering sub-group for which I had lead responsibilities grew from four to six engineers. The two headcounts describe different organizational units and must not be combined.

### Technical Lead Responsibilities

Alongside individual project delivery I took on lead responsibilities for a separate internal engineering sub-group, including responsibility for the engineers' professional development and performance evaluation, while maintaining roughly 80% individual-contributor output. That internal group grew from four engineers to six and was not the same as the SIP project delivery team, so onboarding and mentoring became a recurring rather than occasional part of the role.
...
- My WebRTC/mediasoup work covered phone-side integration and debugging plus a limited contribution to the initial JavaScript/Node.js server implementation; I did not own the later server rewrite.
...
- **My work:** phone-side Qt/Linphone video flows plus WebRTC/mediasoupclient/libsdptransform/libyuv integration, target debugging and a limited contribution to the initial JavaScript/Node.js conferencing-server implementation.
- **Ownership boundary:** lead with the phone-side integration; describe the early server work as a limited contribution and do not imply ownership of the later server rewrite.
