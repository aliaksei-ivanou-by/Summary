...
| Contribution status | Backup-server application integration was merged; the AuthInfo `DomainBackup` change and CallSession/CallService refactor were completed as engineering work; Autoprovisioning Manager release history reaches 2.13.1 |
...
| Customized Linphone/liblinphone | SIP registration/calls and project-specific backup/active-server behavior | C/C++ API and callbacks | Application integration plus an AuthInfo backup-domain SDK change; the broader watchdog/failback state machine was shared or inherited SDK functionality | Present in `cumanphone` and `linphone-sdk` history |
...
Customer-side staffing varied during the project, so the figures above describe the typical team composition.
...
- My primary/backup work included the original Qt-level rotation attempt and its reversion, migration to customized liblinphone APIs, active-proxy diagnostics, settings and backward-compatibility work, and a separate SDK AuthInfo backup-domain change.
- The complete SDK watchdog/failback state machine was a shared or inherited SDK responsibility.
...
The project used a customized Linphone SDK/liblinphone layer rather than an untouched upstream SDK. I integrated non-upstream backup/active-server APIs from the Qt application and made targeted SDK changes; upstream Linphone and the rest of the project fork were outside my ownership.
...
The customized SDK exposed concepts such as primary, backup and active server address, current backup state and watchdog/failback behavior. That state belongs close to registration rather than in a UI retry loop. My responsibility covered application integration and the AuthInfo backup-domain change; the complete watchdog, cooldown, persistence and forced-failback implementation belonged to the broader SDK implementation.
...
- **Ownership boundary:** application integration and AuthInfo changes were mine; the complete SDK watchdog/failback state machine was shared or inherited functionality.
...
- **Result:** The final application integration stopped maintaining a separate manual switch loop and instead synchronized UI/configuration around SDK-reported state. The application work was merged.
- **Status / evidence:** `cumanphone` history contains the application-level prototype, its reversion and the final SDK-based integration. `linphone-sdk` history contains my `DomainBackup` AuthInfo change. The deeper watchdog/failback internals belong to the broader SDK implementation.
...
- I integrated project-specific primary/backup APIs from customized Linphone and made targeted SDK changes; the complete SDK state machine was a shared or inherited responsibility.
...
| Broader SDK watchdog/failback state machine was shared or inherited | Customized SDK implementation beyond the application and AuthInfo changes | **Ownership boundary** | Broader SDK responsibility |
