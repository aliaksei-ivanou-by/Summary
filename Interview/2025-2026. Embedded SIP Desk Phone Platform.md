...
- **Accuracy check before reuse:** sections 05, 10 and 13.
...
| Team | Recalled staffing: 4 C++ engineers on our side; customer PM, approximately 2-4 C++ developers, 1 web developer and usually 2-4 QA engineers; exact period-by-period staffing is to verify |
| Users / deployment / scale | Managed office, reception, dispatcher, meeting-room and contact-center scenarios; deployed-device count not verified |
| Main stack | C++17, Qt 5.15, Linphone/liblinphone, SIP/RTP, Yocto, Embedded Linux, C#/.NET 8, WinForms |
| Primary ownership | Qt call/account architecture; application-side integration of customized Linphone primary/backup APIs; a backup-domain AuthInfo SDK change; major Autoprovisioning Manager implementation; cross-layer device debugging |
| Product status | Publicly listed phone models; release status varies by software feature |
| Contribution status | Backup-server application integration was merged; inclusion of the AuthInfo `DomainBackup` patch and late CallSession/CallService refactor in a product release was not established when I left the project; Autoprovisioning Manager release history reaches 2.13.1 |
...
I worked on the software stack for an embedded SIP desk-phone platform, primarily as a C++/Qt engineer. The product combined a touchscreen application, a customized Linphone SDK, Yocto firmware and a Windows provisioning tool. My strongest areas were call/account architecture, integrating primary/backup SIP-server behavior through the application and provisioning paths, and building out most of the practical Autoprovisioning Manager functionality. The hardest part was keeping asynchronous SIP state, UI models, persisted configuration, firmware services and physical-device behavior consistent across layer boundaries.
...
A representative problem was primary/backup SIP-server support. It was not enough to store two addresses: the system had to expose the active server, keep registration and authentication context coherent, update the Qt UI and persist/generate compatible configuration. An early application-level switching approach was reverted; I then integrated the phone with the customized liblinphone backup/active-server APIs, added active-proxy diagnostics and compatibility handling, contributed a backup-domain field to SDK AuthInfo, and aligned the settings and provisioning paths. The deeper SDK watchdog/failback state machine existed behind those APIs, but the available repository evidence does not support claiming all of its internals as my sole implementation.
...
Public product and export-catalog entries list both ITL780 and ITL789 as market-facing Cuman products. The linked third-party registry page lists EAEU notification KZ0000007309 for the ITL780 as active and links an official EAEU record. This is not evidence that I performed certification, that every firmware build was certified or that the third-party page is authoritative; verify the official record before using the claim externally. TLS/SRTP/HTTPS support is listed separately in Cuman's public product specifications.
...
Public sources describe Cuman/DSS equipment generally as used by government and large-infrastructure organizations. This is company context only; it is not evidence that a named organization deployed the ITL780/ITL789 models or the particular software build I worked on. Listing individual customer names adds little interview value and creates an avoidable risk of implying a deployment claim.
...
- Public model specifications provide feature scale such as programmable keys and conference capacity; they do not establish deployed fleet size, traffic volume or adoption of a particular firmware build.
- The number of deployed devices, customer sites, supported PBX products and measured availability/recovery targets is not documented here.
...
### Cross-Cutting Requirements and Security Caveats
...
- **Performance/resources:** ARM64 hardware, media conversion/rendering and full-image build cycles constrained debugging and iteration; no defensible CPU, memory or latency KPI is documented.
- **Trusted-network protocols:** DHCP, TFTP and SNMP v2/v2c are operationally useful for managed LAN provisioning/discovery but do not provide modern end-to-end confidentiality and authentication. Do not present them as secure transport mechanisms.
...
- **Project-file encryption:** `.cuman` files are AES-encrypted with an application-held key and a generated IV stored with the ciphertext. The implementation does not add an AEAD authentication tag or separate MAC, so it must not be presented as tamper-evident/authenticated storage, a user-keyed credential vault or a complete secret-management design.
- **Secure-media claims:** TLS/SRTP/ZRTP/HTTPS support should be stated as product/SDK capability or integration work, not as proof that every deployment enabled or formally certified it.
...
Where no historical numeric target was recorded, this document uses observable behavior and repeatable scenarios rather than inventing a metric.

### Public Sources for This Section

Use public sources only for customer, product and market-context claims. Do not use them to overstate private implementation details. Cuman pages are vendor sources, export-catalog pages are market listings, and Glavcert is a third-party registry view.
...
- Official EAEU record linked by Glavcert: [https://nsi.eaeunion.org/portal/1994/card/0455d523-76aa-45d1-94ac-91e5ca3f39f2](https://nsi.eaeunion.org/portal/1994/card/0455d523-76aa-45d1-94ac-91e5ca3f39f2) (verify before external use)
...
| Customized Linphone/liblinphone | SIP registration/calls and project-specific backup/active-server behavior | C/C++ API and callbacks | Integration plus an AuthInfo backup-domain SDK change whose inclusion in a release was not established; full internal failover ownership requires caution | Partially verified across `cumanphone` and `linphone-sdk` history |
...
| Yocto image/runtime | Builds and installs binaries, services, dependencies and model assets | BitBake recipes, systemd, filesystem layout | Recipe/package/startup contributions and target debugging | Verified in related layer history; quantify per change before quoting |
...
- **Owned or directly implemented:** application-side primary/backup SIP integration and diagnostics, a backup-domain AuthInfo SDK change, major areas of the Windows Autoprovisioning Manager, and multiple Qt application models/controllers including late-project call-architecture refactoring.
...
|---|---|---|---|
| Embedded application | C++17, Qt 5.15, Qt Widgets, Qt Multimedia, Qt Network, Qt DBus, SQLite | Existing touchscreen application and device-integration foundation | Primary development area |
| Telephony and media | Linphone SDK/liblinphone, SIP, RTP/RTCP, SRTP/ZRTP-related integration, WebRTC/mediasoup, libyuv | Product telephony and conferencing requirements | Application integration and targeted SDK changes; not ownership of upstream Linphone |
| Firmware/platform | Yocto Kirkstone, BitBake, Embedded Linux, systemd, Wayland/Weston, i.MX ARM64 | Reproducible BSP, filesystem and service integration | Recipes, packaging, startup and target debugging |
...
|---|---|---|---|---|
| Primary/backup registration | Keep manual switching/retry logic in Qt | Use customized liblinphone APIs as the authority for active/backup state and let Qt synchronize presentation/configuration | Dependency on a customized SDK and rebase cost | I implemented and then reverted an application-level rotation attempt, integrated the SDK APIs and added diagnostics/compatibility handling; the final application integration was merged |
...
| Late call-architecture refactor | Continue expanding the legacy call model/facades | Introduce explicit session, service, adapter and presentation boundaries | Large migration surface and release risk near project end | I implemented the refactor as internal work; inclusion in a product release was not established when I left the project |
...
These are remembered approximate ranges, not a repository-derived staffing audit. Use approximate wording unless project records confirm exact numbers for the period being discussed.
...
- I can explicitly claim application-side primary/backup integration: the original Qt-level rotation attempt and its reversion, migration to customized liblinphone APIs, active-proxy diagnostics, settings/backward-compatibility work, and a separate SDK AuthInfo backup-domain change.
- I should not claim sole authorship of the customized SDK's complete watchdog/failback state machine unless the exact SDK revision and commit history provide additional evidence.
- I can claim the late-project CallSession/CallService architecture refactor as engineering work I implemented, but must label its release status as unconfirmed when I left the project.
...
- No repeatable load/soak suite, automated hardware lab or verified historical CPU/memory/latency baseline was available at project end; those remain evidence gaps rather than implied results.
...
**Result and status:** the work contains the new session/service/adapter structure and an incremental migration history, but inclusion in a product release was not established when I left the project. It is valid evidence of architecture and refactoring work; it must not be described as a released customer feature.

**Why it is interview-relevant:** it demonstrates incremental migration of a stateful legacy subsystem, explicit runtime ownership, isolation of a third-party SDK and disciplined separation between domain/session state and Qt presentation. The honest tradeoff is that the change surface was large and the final production status remained unverified at project end.
...
The project used a customized Linphone SDK/liblinphone layer rather than an untouched upstream SDK. I integrated non-upstream backup/active-server APIs from the Qt application and made targeted SDK changes. The safe wording is "customized Linphone integration and targeted SDK patches," not ownership of upstream Linphone or every modification in the fork.
...
My verified implementation history is more precise than "I built the whole failover system":
...
- Added a `DomainBackup` field to SDK AuthInfo; the available history verifies my change but does not establish its inclusion in a released SDK.
- Aligned the Windows provisioning configuration with primary/backup fields and authentication context.

The customized SDK exposed concepts such as primary, backup and active server address, current backup state and watchdog/failback behavior. That state belongs close to registration rather than in a UI retry loop. However, the available `linphone-sdk` history does not attribute the complete watchdog, cooldown, persistence and forced-failback implementation to me. Those are valid system concepts to explain, but sole personal ownership is **to verify**.

This course correction is a stronger engineering story than claiming every SDK line: the first solution duplicated registration state in the application; the final integration consumed SDK-owned state and reduced competing state machines.
...
Not every change in that list was mine. The available personal SDK history includes the AuthInfo backup-domain change and a small H.26x NAL-packer update. Broader audio/video conference, ticker and clock-source claims should be tied to a specific change or task before being presented as personal work.
...
The later fix made project save/load more explicit. The code uses a dedicated `Project` model with nested `SipAccountUIState` and `BlfButtonState` data, serializes that state as JSON, encrypts it with AES in the `.cuman` file, and restores the UI through explicit `GatherUIState` / `RestoreUIFromState` logic instead of relying only on generic control-tree serialization. This made the supported format easier to reason about when new fields were added. It did not, by itself, provide a migration path for every earlier payload. Because the key is application-held rather than user-derived and the implementation has no AEAD tag or separate MAC, the safe claim is encrypted project container, not authenticated/tamper-evident storage or a hardened credential vault.

Several later changes had to be checked against this project-file format: expanding the tool from 5 to 10 SIP accounts (release 2.9.0), adding Password/HA1 authentication-mode state, adding screen backlight, saving microphone-off state and changing remote-contact update-interval semantics. The 10-account figure is verified for the provisioning tool release history, but should not be generalized into a phone-model product claim without firmware/product evidence. The important compatibility risk was that a project from another schema revision could open with missing/default values, be rejected, or omit a field that existed in the UI. The available implementation has no explicit multi-version migration layer or historical fixture suite.
...
| Early .NET 8 JSON `FormState` payload | No explicit fallback/migrator was found after the reader changed to `Project`; field-level compatibility is not established | **To verify** with a real fixture |
| AES + JSON `Project` payload used at project end | Explicit gather/save/open/restore path exists; semantic round-trip is the safe validation claim | **Verified implementation**; automated fixture coverage not established |
| Generated provisioning archive | Intended to remain a stable firmware-facing contract across tool changes | **Partially verified** by code/history; historical golden-file matrix not found |
...
**Feature-status caveat:** realm probing was expanded in releases 2.8.0/2.8.3 and selectable Password/HA1 mode was added in 2.10.0. Release 2.10.1 then temporarily disabled the HA1 selection UI and locked accounts to Password mode because of issues in the new flow. The available release notes through 2.13.1 do not record a re-enable. It is accurate to claim implementation and debugging of the HA1/realm feature, but not to claim that selectable HA1 mode was enabled when I left the project.
...
The project release notes state that the built-in HTTP distribution server was removed in 2.3.0 and TFTP became the tool's configuration-distribution server. This is another place where protocol capability and release status should not be conflated.
...
- **Status:** controller work exists in the product code; the broader late-project refactor was internal work whose inclusion in a product release was not established.
...
- **Core insight:** failover is domain state, not a Qt retry timer. The application should consume SDK-owned active/backup state instead of running a competing state machine.
- **My work:** application-level prototype and reversion, customized liblinphone API integration, active-proxy diagnostics, backward compatibility, an AuthInfo backup-domain change and provisioning alignment.
- **Residual uncertainty:** sole ownership of the SDK's complete watchdog/failback internals is not established by the available history.
...
- **Status caveat:** the 10-account tool support is recorded in 2.9.0, while selectable HA1 mode was disabled in 2.10.1 and not recorded as re-enabled before I left the project.

## 09 Selected STAR / Debugging Stories

These four stories have identifiable personal actions and repository evidence. The first three are present in merged or released history; the architecture-refactor story must retain the caveat that its inclusion in a product release was not established.

### 1. Replace Competing Qt Failover Logic with SDK-Owned State

- **Situation:** Primary/backup SIP support crossed account settings, active registration state, authentication, UI diagnostics and provisioning. An initial application-level rotation approach duplicated responsibility already moving into the customized SDK.
...
- **Actions:** I implemented the first Qt-level rotation attempt, reverted it, then migrated the application to customized liblinphone backup/active-server APIs. I updated account/settings paths, added IPv6-aware parsing and active-proxy logging, preserved fallback to the legacy `backup_server` parameter, contributed `DomainBackup` to SDK AuthInfo and aligned provisioning fields.
- **Result:** The final application integration stopped maintaining a separate manual switch loop and instead synchronized UI/configuration around SDK-reported state. The application work was merged; no defensible availability or recovery-time KPI was found.
- **Status / evidence:** `cumanphone` history verifies the application-level prototype, its reversion and the final SDK-based integration. `linphone-sdk` history verifies my `DomainBackup` AuthInfo change but does not establish its inclusion in an SDK release. Exact ownership and release status of deeper watchdog/failback internals remain **to verify**.
...
- **Result:** Persistence became explicit and reviewable; new fields had a defined schema location and the supported format could be checked through a semantic round trip: save, reopen and regenerate meaningful provisioning output. Pre-.NET 8 `BinaryFormatter` files remained incompatible, no general historical migration guarantee is claimed, and defect-rate reduction was not measured.
...
- **Status / evidence:** Autoprovisioning Manager commit `12d0000`, README 2.8.3 and the `GetSipRealmAsync` implementation. Selectable HA1 mode was later disabled in 2.10.1 and was not recorded as re-enabled through 2.13.1.
...
- **Result:** The internal work contains a substantially clearer session/service/adapter structure and a traceable incremental migration. It is architectural implementation evidence, not a shipped outcome.
- **Status / evidence:** The available history and files verify the refactor itself, but do not establish its inclusion in a released product when I left the project.
- **Tradeoff / lesson:** Incremental adapters reduced migration risk and kept intermediate states buildable, but temporarily increased indirection and left release integration unfinished at project end.
...
These are useful follow-up examples, but they need a specific personal action plus merge/release/result evidence before becoming STAR stories:
...
The project did not maintain a single set of historical product KPIs that I can safely quote. The strongest outcomes are therefore stated as delivered behavior with an explicit evidence boundary.

| Outcome | Engineering / user impact | Evidence | Confidence |
|---|---|---|---|
| Qt application integrated with customized primary/backup and active-server APIs | Removed the competing application-level switch loop and aligned settings/diagnostics with SDK-reported state | `cumanphone` commits `6ac7f2a4`, `7e31837f` and related merged history; SDK AuthInfo commit `03d6a1c7` | **Verified implementation**; full SDK ownership and availability improvement are **to verify** |
| Windows Autoprovisioning Manager migrated from .NET Framework 4.8 to .NET 8 | Put the tool on the intended modern runtime while retaining its desktop/configuration workflow | Personal commit `b6a93c1`, `net8.0-windows` SDK-style projects and README 2.0 | **Verified implementation**; operational benefit is **inferred**, not measured |
| Explicit AES-encrypted `.cuman` project-state persistence | Removed hidden dependence on generic UI-tree serialization and made schema growth reviewable | Personal commit `c1d0768`, `Project`/nested state types, JSON/AES and explicit gather/restore logic | **Verified implementation**; historical defect-rate reduction is unknown |
| Discovery, configuration generation and DHCP/TFTP delivery combined in one tool | Supports repeatable common and MAC-specific provisioning workflows | README workflow/releases; discovery/SNMP, configuration-builder, archive, DHCP and TFTP code | **Verified capability**; adoption and time saved per device are unknown |
| Realm probe accepts multiple real-world SIP challenge formats | Reduced dependence on one server's 401/header layout and improved diagnosability | Commit `12d0000`, README 2.8.3 and parser implementation | **Verified implementation**; universal PBX interoperability is not claimed |
| Late call subsystem refactored toward sessions, services, adapters and presentation boundaries | Demonstrates a clearer ownership model and incremental legacy migration | Related implementation, tests and architecture documentation | **Verified internal work**; release impact is unknown |
| Application, SDK, services and assets integrated into model-specific Yocto images | Enabled full-image and physical-device validation rather than testing only disconnected binaries | BitBake metadata, installed layout, service definitions, firmware builds and target scenarios | **Verified contribution area**; quantify personal/released changes before using as an impact claim |

### Metrics Worth Collecting Before Reusing This for a Resume

- Number of personally completed tasks, fixes or merged changes by subsystem.
- Number of device models and firmware versions on which the key scenarios were validated.
- PBX/SIP-server interoperability matrix used for registration, failover, calls and authentication.
- Provisioning time or number of manual steps before and after the manager workflow.
- Number of configuration fields/accounts supported by released builds, separated from internal build scope.
- Qt Test inventory and effective test coverage; the Autoprovisioning Manager repository had 70 MSTest methods at project end, but no coverage percentage was established.
- Any support data showing reduced recurrence or diagnosis time for provisioning, startup or registration failures.

Do not turn these into resume metrics until a repository, task tracker, release note or stakeholder can substantiate them.

### Resume-Ready Bullets

- Integrated customized liblinphone primary/backup APIs into a C++/Qt phone application, replacing a competing application-level switch loop and adding active-server diagnostics, compatibility handling and aligned provisioning fields.
...
- Refactored a stateful Qt call subsystem toward explicit sessions, services and Linphone adapters through an incremental internal workstream; do not imply that it shipped.
...
- I initially implemented primary/backup rotation in the Qt application. Once customized liblinphone APIs became the authoritative source for active/backup state, I reverted that approach and integrated the SDK model instead. The lesson was to place registration lifecycle state beside registration, not duplicate it in the presentation/application layer.
...
- The late call-architecture refactor improved boundaries but grew into a large internal migration near project end. A clearer release-integration plan and earlier vertical slices would have reduced uncertainty about shipment status.
...
For a resume or interview, the safest comprehensive version is:

"Worked from May 2025 to August 2026 on software for a commercially listed embedded SIP/IP desk-phone platform. My main areas were C++/Qt call and account architecture, integration with customized Linphone primary/backup APIs, major implementation and .NET 8 modernization of a Windows Autoprovisioning Manager, Yocto-based image/runtime integration, and cross-layer debugging on physical ARM64 phones. I also implemented SIP realm-probe compatibility and configuration/persistence fixes across provisioning and device runtime."
...
- "Implemented the entire Linphone failover/watchdog state machine."
- "Shipped the CallSession/CallService refactor to customers."
- "HA1 selection was enabled when I left the project."
- "DHCP/TFTP/SNMP provisioning is secure by itself."
- "The `.cuman` format is a hardened credential vault."
- "`.cuman` files are backward compatible across the .NET Framework and .NET 8 formats."
- "Selected .NET/WinForms, Qt 5/Qt Widgets or Yocto as greenfield technology choices."
- "Replatformed the firmware build system."

Useful precise alternatives:

- "Integrated project-specific primary/backup APIs from customized Linphone and made targeted SDK changes" instead of claiming the entire SDK state machine.
- "Integrated with the device-local REST/API and web configuration path" instead of "implemented the REST API or web UI."
- "Worked across the embedded product stack" instead of "owned the entire stack."
- "Adapted the inherited Yocto platform and recipes" instead of "selected or introduced Yocto."
- "Implemented a late-project call-architecture refactor whose inclusion in a product release was not established" instead of implying customer shipment.
- "Implemented HA1/realm functionality; HA1 selection was later temporarily disabled in the available project-end release history" instead of implying availability at project end.
- "AES-encrypted application project file" without implying user-derived keys, cryptographic tamper detection or enterprise secret-management guarantees.
- "Round-trip validation for the supported `Project`/JSON format" instead of claiming compatibility with every historical `.cuman` payload.

### Strongest Technical Deep Dives

- **Primary/backup SIP integration:** why the first Qt-owned switch loop was reverted, how the app consumed SDK active/backup state, and where authentication/configuration had to remain consistent.
- **Call architecture refactor:** incremental extraction of session/runtime/services/adapters from a legacy UI-facing model, with the unconfirmed release status stated up front.
...
- **Could the tool version at project end open every old `.cuman` file?** No. The pre-.NET 8 encrypted `BinaryFormatter` payload is explicitly treated as incompatible by the JSON reader, and no general migration path for every intermediate schema was found. The defensible claim is round-trip behavior for the supported `Project`/JSON format.
...
- **Did the call refactor ship?** The implementation is verified, but its inclusion in a released product was not established when I left the project, so I would not claim customer shipment.
- **Was HA1 selection enabled when I left the project?** The capability and realm parser were implemented, but README 2.10.1 records the selection UI as temporarily disabled and no re-enable is recorded through 2.13.1.
...
- Public customer and product sources are listed in section 02. They support product context, not personal implementation claims, and should be verified before external use.
...
|---|---|---|---|
| Main phone app uses C++17 and the listed call controllers/media libraries | `CumanPhone.pro`, controller sources and tests | **Verified** | Safe internal technical detail |
| Backup-server application integration is personal and merged | `cumanphone` commits `4a2a9442`, `6ac7f2a4`, `7e31837f` and related history | **Verified** | Safe without private URLs/configuration |
| AuthInfo `DomainBackup` SDK patch exists and was personally authored | Commit `03d6a1c7`; inclusion in an SDK release is not established | **Verified implementation; release status unknown** | State the release uncertainty |
| Complete SDK watchdog/failback internals were personally implemented | Available SDK history does not prove sole ownership of the complete state machine | **To verify** | Do not claim yet |
| CallSession/CallService refactor was personally implemented | Related source, tests, documentation and repository history; inclusion in a product release is not established | **Verified internal work; release status unknown** | Do not imply customer shipment |
| Autoprovisioning Manager targets .NET 8 | Application/test `.csproj` and README 2.0 | **Verified** | Safe |
| `.cuman` persistence refactor was personal released work | Commit `c1d0768`, release `v2.4.1` and later `Project`/UI-state changes | **Verified** | Safe with encryption and legacy-format caveats |
| Pre-.NET 8 `.cuman` files remain readable after migration | `b6a93c1^` used `BinaryFormatter`; `b6a93c1` switched to JSON and reports the old format as incompatible | **Contradicted** | State the compatibility boundary explicitly |
| Tool expanded from 5 to 10 SIP accounts | Personal commit `44e947a`, README 2.9.0 and ten-account UI/state loops | **Verified tool feature** | Do not generalize to all phone firmware |
| Realm parser supports 401/407 and common header variations | Commit `12d0000`, README 2.8.3 and `GetSipRealmAsync` | **Verified** | Safe |
| Selectable HA1 mode was enabled at project end | README 2.10.1 disables it; no re-enable appears through 2.13.1 | **Contradicted by available release history** | State only as implemented, then disabled |
| Autoprovisioning tests exist at meaningful scale | 70 MSTest methods in the repository at project end | **Verified repository metric** | Not test coverage or executed-pass count |
| Product models/specifications and company context | Public URLs in section 02 | **Public vendor/market context** | Verify before external reuse |
| EAEU notification KZ0000007309 is listed as active | Glavcert record and linked official EAEU source | **Third-party status; official verification required** | Do not present as my certification work |

### Open Questions Before the Interview

- Which firmware/release identifiers contain each major contribution?
- Which product models and PBX/SIP servers were personally validated for each story?
- Is there a defensible before/after measure for provisioning effort, registration recovery or support diagnosis time?
- Which exact customized SDK revision contains the watchdog/failback implementation, and which parts of that history are personally attributable?
- Which `.cuman` versions need a supported migration path, and are representative historical files available as fixtures?
- Which other features reached released customer firmware versus an internal build only?
- Which three stories best match the target role: embedded C++, Qt architecture, SIP/VoIP, Yocto or desktop tooling?
